...
export default class CharityEdit extends Component<Props, State> {
    ...
    public componentDidMount(): void {
        ...
        this.cancelCharityTypes = CharityType.builder().subscribe(async (cts) =>
            this.setState({ charityTypes: cts })
        );
    }
            ...
            <Content>
                <FullWidth className={styles.header}>
                    ...
                    <div className={styles.info}>
                        <Input
                            ...
                            onChange={addValue(
                                bindArgs("longName", this.charityChangeString)
                            )}
                        />
                        <Input
                            white
                            ...
                            onChange={addValue(
                                bindArgs("tagline", this.charityChangeString)
                            )}
                        />
                        <Input
                            white
                ...
                <FullWidth className={styles.social}>
                    <Input
                        ...
                        onChange={addValue(
                            bindArgs("facebookUrl", this.charityChangeString)
                        )}
                    />
                    <Input
                        white
                        ...
                        onChange={addValue(
                            bindArgs("twitterUrl", this.charityChangeString)
                        )}
                    />
                    <Input
                        white
                        ...
                        onChange={addValue(
                            bindArgs("websiteUrl", this.charityChangeString)
                        )}
                    />
                    <Input
                        white
                        ...
                        onChange={addValue(
                            bindArgs("canadaHelpsUrl", this.charityChangeString)
                        )}
                    />
                    <Select
                        title="Charity Type"
                        options={this.state.charityTypes.map((ct) => [
                            ct.id ?? "",
                            ct.name,
                        ])}
                        value={this.state.charity.type?.id ?? ""}
                        onChange={addValue((value: string) => {
                            const type = this.state.charityTypes.find(
                                (ct) => ct.id === value
                            );
                            if (type) {
                ...
                <TabContainer>
                    <Tab title="Donation Targets">
                        <Button color="dark" onClick={this.targetAdd}>
                            Add
                        </Button>
                        <div className={styles.donationTargets}>
                            {charity.donationTargets.map((target, i) => (
                                <div className={styles.target} key={i}>
                                    ...
                                    <Button
                                        color="danger"
                                        onClick={bindArgs(i, this.targetDelete)}
                                    >
                                        Delete
                                    </Button>
                                </div>
                    ...
                    <Tab title="About Charity">
                        ...
                        <textarea
                            ...
                            onChange={addValue(
                                bindArgs("longDesc", this.charityChangeString)
                            )}
                        />
                        ...
                        <div className={styles.extraInfo}>
                            ...
                            <Input
                                ...
                                value={charity.phone}
                                onChange={addValue(
                                    bindArgs("phone", this.charityChangeString)
                                )}
                            />
                            <Input
                                title="Fax"
                                value={charity.fax}
                                onChange={addValue(
                                    bindArgs("fax", this.charityChangeString)
                                )}
                            />
                            <Input
                                title="Email"
                                value={charity.email}
                                onChange={addValue(
                                    bindArgs("email", this.charityChangeString)
                                )}
                            />
                            <h4>Office Hours</h4>
                            <Input
    ...
    @bind
    private charityChangeString(
        field: FilterPropertyNames<Charity, string>,
        value: string
    ): void {
        this.setState((state) => {
        ...
        try {
            ...
            if (this.props.match.params.id !== this.state.charity.id) {
                this.props.history.push(
                    `/charity/${this.state.charity.id}/edit`
                );
            }
    ...
    private async changeIcon(index: number): Promise<void> {
        const ChangeIcon: FunctionComponent<{
            ctl: ModalControl<IconName | undefined>;
        }> = ({ ctl }) => {
            const [search, changeSearch] = useState("");

            return (
                <Modal className={styles.iconChange} onCloseClick={ctl.close}>
        ...
        const icon = await openModal<IconName | undefined>(
            (ctl) => <ChangeIcon ctl={ctl} />,
            undefined
        );
        if (icon) {
            this.setState((state) => {
