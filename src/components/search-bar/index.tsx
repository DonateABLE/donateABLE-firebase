...
const SearchBar: FunctionComponent<Props> = (props) => {
    ...
    const enableCharityType = useCallback(
    ...
    const disableCharityType = useCallback(
        (charityTypeID: string) => {
            props.onChange({
                ...props.value,
                charityTypes: [
                    ...props.value.charityTypes.filter(
                        (ct) => ct.id !== charityTypeID
                    ),
                ],
            });
        },
    ...
    return (
        <div className={styles.searchBar} ref={popup}>
