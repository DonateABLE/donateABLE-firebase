    ...
    return (
        <Fragment>
            <FullWidth className={styles.header}>
                <div
                    className={classNames(styles.logo, props.imageClassName, {
                        [styles.round]: props.imageRound,
                    })}
                >
                    <img
                        src={props.image}
            ...
            <HeaderButton
                href={props.buttonLocation}
                className={styles.makeDonationLower}
            >
                {props.buttonTitle}
            </HeaderButton>
        </Fragment>
...
const HeaderButton: FunctionComponent<{ href: string; className: string }> = (
    props
) => {
    const internalLink = props.href.startsWith("/");

    const btn = (
        <Button className={props.className} color="dark" size="medium">
            {props.children}{" "}
            {!internalLink && <Icon name="external-link-alt" />}
        </Button>
    );
