...
export const ModalHeader: FunctionComponent = ({ children }) => (
    <h2 className={styles.modalHeader}>{children}</h2>
);
...
export function openModal(
    content: (ctl: ModalControl<void>) => ReactElement
): Promise<void>;
export function openModal<T>(
    content: (ctl: ModalControl<T>) => ReactElement,
    defaultValue: T
): Promise<T>;
...
> {
    ...
    public render(): ReactNode {
        ...
        return this.state.modals.map((v, i) => (
    ...
    public removeModal(modal: ReactElement): void {
        this.setState((pState) => ({
            modals: pState.modals.filter((m) => m !== modal),
        }));
    }
}
